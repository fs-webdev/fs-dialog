import FSDialogBase from './fs-dialog-base.js';
const stylesTemplate = document.createElement('template');
stylesTemplate.setAttribute('style', 'display: none;');

stylesTemplate.innerHTML = `<style data-fs-anchored-dialog="">
...
const pointerTemplate = document.createElement('template');
pointerTemplate.setAttribute('style', 'display: none;');

pointerTemplate.innerHTML = `<div class="fs-dialog-pointer"></div>`;
 ...
 */
const FSAnchoredDialog = class extends FSDialogBase {
  // only used for docs.
   ...
   */
  connectedCallback () {
    this.appendChild(document.importNode(stylesTemplate.content, true));
    this.appendChild(document.importNode(pointerTemplate.content, true));
